...
- LLM adapter layer: an offline structured analyzer by default, with OpenAI-compatible/vLLM/Azure-style, DeepSeek, Zhipu GLM, Kimi/Moonshot, and Ollama adapters for owner-selected large model APIs.
...
DeepSeek, Zhipu GLM, and Kimi/Moonshot are first-class provider types backed by the OpenAI-compatible adapter, with defaults for their official chat-completions endpoints and environment-variable secret references. Ollama is supported through `/api/chat` with JSON output enabled. Anthropic and Gemini can be stored as selectable provider profiles, ready for provider-specific adapters.
...
The plugin dashboard also includes an AI Chat view. It loads connected managed-site agents from `/api/v1/agents` as context, stores conversation history in the `agent_messages` SQLite table, and sends operator questions to `POST /api/v1/ai/chat`. Replies are generated by the currently active LLM adapter: OpenAI-compatible/vLLM/Azure-style endpoints, DeepSeek, Zhipu GLM, Kimi/Moonshot, Ollama, or the offline heuristic adapter. SentinelAI sends compact context containing agent heartbeat, policy version, incident counts, visitor counts, latest incidents, latest visitors, active provider metadata, and the SentinelAI scoring framework with identity, behavior, runtime, and asset-impact layers. The compatibility route `/api/v1/agent/chat` is still accepted.
...
The UI provides English and Chinese language options. Its visual treatment is a high-contrast tactical clue-map console inspired by the supplied screenshot: dark mineral terrain, translucent orbit rings, prism light beams, lime geometric shards, clipped HUD panels, and circular progress nodes arranged around the centered real-time monitoring surface. It uses local HTML, CSS, JavaScript, and a local `mission-map.json` configuration file; it does not copy brand assets or protected artwork and does not depend on external CDN libraries.

The dashboard views are:

- Monitor: real-time metrics, live incidents, and visitor stream.
- Monitor clue map: JSON-configured progress nodes that jump directly to product modules.
